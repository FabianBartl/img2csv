...
def process_single_cell(cell, img, custom_config, stop_event):
    """Worker function to process OCR for a single table cell with cancellation check."""
...
def run_ocr():
    ...
    def generate():
        ...
        try:
            futures = [
                executor.submit(process_single_cell, cell, img, custom_config, stop_event)
                for cell in cells
            ]
            ...
            print('Client disconnected or canceled request. Halting backend OCR tasks.')
        finally:
            # Signal worker threads to stop and purge remaining queued tasks
            stop_event.set()
